...
vector* MLFQ(process* process_arr, int n, int max_level)
{
	...
	while (index < n || all_q_empty == 0)
	{
		...
		excute = Schedueling(lq, excute, &excute_time, max_level);
		if (excute != 0)
		{
...
process* Schedueling(processqueue* lq, process* previous_process, int* excute_time, int max_level)
{
	...
	{
		if ((1 << previous_process->priority) == *excute_time)
		{
